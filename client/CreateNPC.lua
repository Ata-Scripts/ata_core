...
-- Function to add targets to an existing NPC
function AddTargetsToNPC(npc, targets)
    if not targets or type(targets) ~= "table" then
        print("^1[ata_core] ERROR: targets parameter must be a table!^0")
        return
    end
    ...
    if Config.Target == 'ox_target' then
        ...
        for i, target in ipairs(targets) do
            table.insert(options, {
                name = 'npc_interact_' .. i,
                label = target.text or "Interact",
                icon = target.icon or 'fa-solid fa-user',
                onSelect = function()
                    if target.eventType == "server" then
                        if target.eventData then
                            TriggerServerEvent(target.eventName, target.eventData)
                        else
                            TriggerServerEvent(target.eventName)
                        end
                    else
                        if target.eventData then
                            TriggerEvent(target.eventName, target.eventData)
                        else
    ...
    elseif Config.Target == 'qb-target' then
        ...
        for i, target in ipairs(targets) do
            table.insert(options, {
                num = i,
                type = target.eventType or "client",
...
exports("AddTargetsToNPC", AddTargetsToNPC)
